...
void sumPoints(float * sum_cluster, int * pointsCluster, float * points){
    for(int i = 0; i < N; i++){
        switch (pointsCluster[i])
        {
            case 0:
                sum_cluster[0]+= points[i * 2];
                sum_cluster[1]+= points[i * 2 + 1];
                break;
            case 1:
                sum_cluster[2]+= points[i * 2];
                sum_cluster[3]+= points[i * 2 + 1];
                break;

            case 2:
                sum_cluster[4] += points[i * 2];
                sum_cluster[5]+= points[i * 2 + 1];
                break;


            case 3:
                sum_cluster[6] += points[i * 2];
                sum_cluster[7] += points[i * 2 + 1];
                break;
